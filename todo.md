...
- [ ] Добавить `Case`-узлы в `investigation`: использовать `case_claims/case_events` как отдельный evidence-layer `Entity -> Claim -> Case -> Event/Content`, чтобы кейсы перестали быть только экспортным представлением в Obsidian
- [ ] Обогатить `contracts/contract_parties` второй стороной сделки: текущие live-данные `government_contract` дают только `30` party-rows на `30` контрактов, то есть все контракты пока односторонние и не позволяют строить `Organization -> Contract -> Counterparty`
