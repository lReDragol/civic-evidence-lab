...
- [x] Расширить structural seeding beyond contracts: `same_vote_pattern`, `same_bill_cluster` и консервативный `same_case_cluster` теперь живут в `relation_candidates` без возврата старого co-occurrence шума; после повторного nightly `weak_similarity` в snapshot = `729`, а не `5912`
...
- [x] Добавить Windows Task Scheduler bootstrap/install script для автозапуска `runtime.daemon` при старте машины: `python -m runtime.task_scheduler` теперь пытается создать `schtasks` entry, а при `Access is denied` автоматически ставит launcher в user Startup folder
- [x] Построить gold-set / `classifier_audit_samples` pipeline и automatic drift gate: nightly теперь прогоняет `classifier_audit` до `analysis_snapshot/obsidian_export`, сохраняет `300/300/300` samples, baseline versioning и блокирует публикацию при реальной деградации
- [x] Добить live OCR runtime-совместимость на этой машине: `media_pipeline/ocr.py` переключается с `PaddleOCR` на `RapidOCR + ONNXRuntime`, live batch обработал `100` вложений, записал `83` OCR-текста и оставил только `12` `ocr_missing_attachment`
