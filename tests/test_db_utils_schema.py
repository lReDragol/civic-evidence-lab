    ...
    def test_exec_schema_upgrades_legacy_db_before_new_indexes(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            try:
                ...
                self.assertIn("task_type", columns("ai_task_attempts"))
                self.assertIn("latency_ms", columns("ai_task_attempts"))
                self.assertIn("tokens_in", columns("ai_task_attempts"))
                self.assertIn("tokens_out", columns("ai_task_attempts"))
                self.assertIn("estimated_cost", columns("ai_task_attempts"))
                self.assertIn("current_item_key", columns("ai_task_attempts"))
